...
Originally copied from Harbor's SingularityEnvironment, converted to fit BenchFlow's sandbox interface.
Notable changes:
- The usage of the --fakeroot flag is now configurable. By default it is not used.
- /usr/bin:/usr/local/bin: is no longer prepended to PATH.
- The bootstrap script features additional setup commands, e.g. fixing various issues with apt installs.
"""
...
class SingularitySandbox(BaseSandbox):
    """
    ...
    def __init__(
        ...
        singularity_extra_mounts: list[Mount] | None = None,
        singularity_no_mount: str | None = None,
        singularity_use_fakeroot: bool = False,
        **kwargs,
    ):
        ...
        self._extra_mounts = singularity_extra_mounts or []
        self._singularity_no_mount = singularity_no_mount
        self._singularity_use_fakeroot = singularity_use_fakeroot
    ...
    def _fail_if_service_is_not_main(self, service: str) -> None:
        if service != "main":
            raise ValueError(
                f"Requested service '{service}' is not supported. "
                f"The Singularity sandbox is single-container and does not support services other than 'main'."
            )
    ...
    async def _start_server(self) -> None:
        ...
        for port_attempt in range(max_port_retries):
            ...
            bind_mounts = [
                ...
                # Same mounts as in DockerSandbox
                "-B",
                f"{self.rollout_paths.verifier_dir.resolve().absolute()}:{SandboxPaths.verifier_dir}",
                "-B",
                f"{self.rollout_paths.agent_dir.resolve().absolute()}:{SandboxPaths.agent_dir}",
                "-B",
                f"{self.rollout_paths.artifacts_dir.resolve().absolute()}:{SandboxPaths.artifacts_dir}",
                # Fixes network issues on some Apptainer versions
                "-B",
            ...
            fakeroot = ["--fakeroot"] if self._singularity_use_fakeroot else []
    ...
    async def start(self, force_build: bool) -> None:
        ...
        # We don't need to upload the environment dir here since all BenchFlow tasks have Dockerfiles.
    ...
    async def exec(
        ...
        self._fail_if_service_is_not_main(service)
